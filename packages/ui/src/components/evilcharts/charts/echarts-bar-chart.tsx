...
import {
  ...
  type ResolvedColors,
} from "@portfolio/ui/components/evilcharts/ui/echarts-chart";
import {
  tooltipBaseOption,
  ...
  type TooltipVariant,
} from "@portfolio/ui/components/evilcharts/ui/echarts-tooltip";
import {
  Brush,
  buildBrushDataZoom,
  syncBrushOverlay,
  ...
  type BrushProps,
  type BrushRange,
} from "@portfolio/ui/components/evilcharts/ui/echarts-brush";
import {
  DataZoomComponent,
  GridComponent,
  TooltipComponent,
...
import { sampleGradient } from "@portfolio/ui/components/evilcharts/ui/echarts-dot";
...
export interface TooltipProps {
  ...
  valueFormatter?: (value: number) => string; // formats numeric values in each tooltip row
...
function shimmerWindowStops(center: number, color: string, peak: number) {
  ...
  for (const offset of offsets) {
    if (stops.length === 0 || offset - (stops.at(-1)?.offset ?? 0) > 1e-4) {
