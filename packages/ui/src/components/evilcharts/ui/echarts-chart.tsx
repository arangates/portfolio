...
export function distributeColors(colors: string[], maxCount: number): string[] {
  ...
  if (available === 0) return Array.from({ length: maxCount }, () => "rgba(120, 120, 120, 1)");
  ...
  for (let i = 0; i < available; i++) {
    ...
    for (let j = 0; j < slots; j++) result.push(colors[i] ?? "rgba(120, 120, 120, 1)");
  }
...
export function withAlpha(color: string, alpha: number): string {
  ...
  const [r = "0", g = "0", b = "0", a] = (match[1] ?? "").split(",").map((p) => p.trim());
