...
function withAlpha(color: string, alpha: number): string {
  ...
  if (m) {
    const [r = 0, g = 0, b = 0] = (m[1] ?? "").split(",").map((s) => parseFloat(s));
...
export function sampleGradient(slots: string[], t: number): string {
  ...
  const [r1 = 120, g1 = 120, b1 = 120, a1 = 1] = parse(slots[index] ?? "rgba(120, 120, 120, 1)");
  const [r2 = 120, g2 = 120, b2 = 120, a2 = 1] = parse(
    slots[index + 1] ?? "rgba(120, 120, 120, 1)",
  );
